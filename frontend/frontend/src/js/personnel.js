  ...
  try {
    ...
    tbody.innerHTML = list.map(p => `
      <tr class="hover:bg-gray-50">
        <td class="p-2 align-top text-xs">${escapeHtml(p.id)}</td>
        <td class="p-2 align-top">${escapeHtml(p.fullName || '')}</td>
        <td class="p-2 align-top text-sm">${escapeHtml(p.phone || '')}</td>
        <td class="p-2 align-top text-right">
          <button class="btn text-xs" data-person-id="${escapeHtml(p.id)}" data-person-name="${escapeHtml(p.fullName || '')}">Выбрать</button>
        </td>
      </tr>
    `).join('');
