  ...
  try {
    ...
    grid.innerHTML = faces.map(f => `
      <div class="relative group border rounded overflow-hidden bg-white">
        <div class="p-1 text-[10px] text-gray-500 break-all">${escapeHtml(f.face_id)}</div>
        <button data-face-id="${escapeHtml(f.face_id)}" class="absolute top-1 right-1 text-[10px] bg-red-600 text-white px-1 rounded opacity-0 group-hover:opacity-100">X</button>
      </div>
    `).join('');
